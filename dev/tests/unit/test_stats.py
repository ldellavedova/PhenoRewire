...
from phenorewire.stats import adaptive_fdr_selection, fast_spearman_permutation_test, spearman_permutation_test
from phenorewire.phenotype_selection import phenotype_selection


def test_fast_spearman_matches_scipy_on_observed_correlations() -> None:
    mat = np.array(
        [
            [1, 2, 3, 4, 5, 6],
            [6, 5, 4, 3, 2, 1],
    ...
    r_obs, p_emp = fast_spearman_permutation_test(mat, target, n_permutations=20, seed=42)
...
def test_exact_and_backward_compatible_alias_match() -> None:
    mat = np.array(
        [
            [1, 3, 2, 4, 5, 8],
            [8, 5, 4, 3, 2, 1],
    ...
    r_exact, p_exact = spearman_permutation_test(mat, target, n_permutations=25, seed=7)
    r_alias, p_alias = fast_spearman_permutation_test(mat, target, n_permutations=25, seed=7)
    assert np.allclose(r_exact, r_alias, equal_nan=True)
    assert np.allclose(p_exact, p_alias, equal_nan=True)
...
def test_empirical_p_low_for_strong_correlation() -> None:
    ...
    r_obs, p_emp = fast_spearman_permutation_test(mat, y, n_permutations=500, seed=99)
...
def test_empirical_p_not_low_for_noise() -> None:
    ...
    r_obs, p_emp = fast_spearman_permutation_test(mat, y, n_permutations=200, seed=7)
